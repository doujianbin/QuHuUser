...
static const CGFloat CSToastHorizontalPadding   = 30.0;  // 文字横距离
static const CGFloat CSToastVerticalPadding     = 15.0;  // 文字纵距离
...
- (void)makeToastActivity:(id)position {
    ...
    activityIndicatorView.center = CGPointMake(activityView.bounds.size.width / 2, activityView.bounds.size.height / 5 * 2);
    [activityView addSubview:activityIndicatorView];
    [activityIndicatorView startAnimating];
    
    UILabel *lab = [[UILabel alloc]init];
    [lab setFrame:CGRectMake(0, activityView.bounds.size.height / 3 * 2 + 5, activityView.bounds.size.width, 16)];
    [activityView addSubview:lab];
    [lab setText:@"加载中"];
